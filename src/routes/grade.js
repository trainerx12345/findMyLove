...
router.get(
  ...
  try {
    const results = await Grade.findOne({ _id: request.params.id }).exec();
    response.json(results);
  } catch (error) {
    response.status(400).json({ error });
  }
...
router.put(
  ...
  async (request, response) => {
    try {
      const obj = await Grade.findOneAndUpdate({ _id: request.params.id }, request.body, {
        new: true,
        upsert: false,
      });
      if (obj) {
...
router.delete("/:id", async (request, respond) => {
  try {
    const success = await Grade.deleteOne({_id:request.params.id}).exec();
    if (success) {
